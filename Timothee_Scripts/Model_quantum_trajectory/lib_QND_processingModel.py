...
from lib_TrajectoryModel import Traject
...
def NbPhotonFromRudat(rudat, a, b):
    return a*rudat + b
...
class Base(object):
    ...
    def __init__(self, dataFolder, with_pio2, tread, rudat, rudatToNbPhoton_Args):
        self._init_dataFolder(dataFolder)

        self.rudat = rudat
        if rudatToNbPhoton_Args is not None:
            self.nbPhoton = NbPhotonFromRudat(rudat, *rudatToNbPhoton_Args)
...
class Loading(Base):
    ...
    def getAdressRawData(self):
        '''
        Takes self.dataFolder to folder. Returns [ list_of_datafiles, list_of_parameter_files ]
        '''
    ...
    def loadTraject_fromAdress(self, type_trj):
        '''
        Main loader trajectories
        Takes list of files adresses and label to mark as 'pi' or 'no'
        Return the list of trajectories as objects of  class Traject
        '''
...
class MeanTraject(Loading):
    ...
    def setSingleThreshold(self):
        ...
        if self.n_aver == 0:
            self.indStartTime = 150
            self.indEndTime = np.argwhere(time_vec == self.durationTraject)[0,0]
...
class BigHistogram(MeanTraject):
    ...
    def setDoubleThreshold(self, definition='Me'):
        nblank = MAX_BLANK
    ...
    def plotHistogramBigTraject(self, log=False, save=True, savepath='', savename='Hist_big_traj.png'):
        ...
        ax.axvline(x=self.threshold_fit, color='peru')
...
class StateDuration(BigHistogram):
    ...
    def plotHistogramDuration(self, state, fit=True, log=False, save=True, savepath='', savename='Hist_dtj.png'):
        nblank = MAX_BLANK_PLOT
        start = time.time()
        sys.stdout.write('Plot of {:{blank}} ...'.format(savename, blank=nblank))
        ...
        dictColor = {'All_e': 'red',  'Censored_e': 'orange',
                     'All_g': 'blue', 'Censored_g': 'green'}
        dictColorFit = {'All': 'black', 'Censored': 'grey'}

        for dtype in ['All', 'Censored']:
            ...
            color = dictColor['{}_{}'.format(dtype, state)]

            #ax.bar is not working for some reason
            #ax.bar(binsDuration, histDuration, log=log, color=color)
            ax.hist(binsDuration, binsDuration, weights=histDuration,
                    color=color, log=log, alpha=0.7, edgecolor='black', label='{} duration'.format(dtype))

            if fit:
                colorFit = dictColorFit[dtype]
                ...
                ax.plot(hist_space, exponential_decay(hist_space, *coeffExp),
                        color=colorFit, label=exponential_decay_label(*coeffExp))

        ax.set_xlabel('$\Delta t_{jump}$ [ns]')
        ax.set_ylabel('Probability')
        ax.set_title(r'Histogram of $|%s\rangle$ duration'%(state))
        ax.legend()
    ...
    def plotCumulativeFrequency(self, state, fit=True, save=True, savepath='', savename='Cumulative_frequency.png'):
        nblank = MAX_BLANK_PLOT
        start = time.time()
        sys.stdout.write('Plot of {:{blank}} ...'.format(savename, blank=nblank))

        listAllDuration = getattr(self, 'listAllDuration_{}'.format(state))
        listCensoredDuration = getattr(self, 'listCensoredDuration_{}'.format(state))
        if listAllDuration is None or listCensoredDuration is None:
            sys.stdout.write('\rPlot of {:{blank}} ...   failed, no data\n'.format(savename, blank=nblank))
            return False

        dividerAll = getattr(self, 'dividerAll_{}'.format(state))
        dividerCensored = getattr(self, 'dividerCensored_{}'.format(state))

        Xcensored = np.sort(listCensoredDuration)
        Ycensored = np.arange(1, len(Xcensored)+1) / len(Xcensored)

        Xall = np.sort(listAllDuration)
        Yall= np.arange(1, len(Xall)+1) / len(Xall)
        ...
        ax.plot(Xcensored, Ycensored, color=colorDiscard, label=r'Censored data')
        ax.plot(Xall, Yall, color=color, label=r'All data')
        ...
        if fit:
            ...
            miniAll = Xall[-1]/dividerAll
            condition = np.where(Xall>miniAll, True, False)

            XAllfit = Xall[condition]
            YAllfit = np.arange(1, len(XAllfit)+1) / len(XAllfit)

            coeffExp = getattr(self, 'coeffFitAllDuration_{}'.format(state))
            coeffExpDiscard = getattr(self, 'coeffFitCensoredDuration_{}'.format(state))

            ax2.plot(XAllfit, YAllfit, color=color)
            ax2.plot(XCensoredfit, YCensoredfit, color=colorDiscard)

            coeff = list(coeffExp) + [miniAll]
            XAllSpace = np.linspace(np.min(XAllfit), np.max(XAllfit), 5000)
            ax2.plot(XAllSpace, cumulativeFrequency(XAllSpace, *coeff),
                     '--', color=color)

            coeff = list(coeffExpDiscard) + [miniCensored]
            XCensoredSpace = np.linspace(np.min(XCensoredfit), np.max(XCensoredfit), 5000)
            ax2.plot(XCensoredSpace, cumulativeFrequency(XCensoredSpace, *coeff),
                     '--', color=colorDiscard)

            ax2.set_title('Left truncated data + fit')

            ax.axvline(x=XAllfit[0], ls=':', color=color)
            ax.axvline(x=XCensoredfit[0], ls=':', color=colorDiscard)


        ax.set_xlabel('$\Delta t_{jump}$ [ns]')
        ax.set_ylabel('Cumulative frequency')
        ax.set_title(r'Cumulative frequency distribution of $|%s\rangle$ duration'%(state))
    ...
    def _plotVariationFit_withNbins(self):
        nblank = MAX_BLANK_PLOT
        start = time.time()
        sys.stdout.write('Plot of {:{blank}} ...'.format('varation fit with Nbins', blank=nblank))

        listNbins = np.arange(10, 70, 1)
        listCoeffFit_e = []
        listCoeffFit_g = []


        for nbins in listNbins:
            hist_e, bins_e, _ = computeHistogram(self.listCensoredDuration_e, nbins=nbins)
            hist_g, bins_g, _ = computeHistogram(self.listCensoredDuration_g, nbins=nbins)

            condition = np.where(hist_e!=0, True, False)
            histFit = hist_e[condition]
            binsFit = bins_e[condition]

            A0 = np.max(histFit[1:-1])
            coeff_e, error = curve_fit(exponential_decay, binsFit[1:-1], histFit[1:-1],
                                       p0=(A0, 1/T1), bounds=(0, +np.inf))

            condition = np.where(hist_g!=0, True, False)
            histFit = hist_g[condition]
            binsFit = bins_g[condition]

            A0 = np.max(histFit[1:-1])
            coeff_g, error = curve_fit(exponential_decay, binsFit[1:-1], histFit[1:-1],
                                       p0=(A0, 1/T1), bounds=(0, +np.inf))

            listCoeffFit_e.append(coeff_e[1]*1000)
            listCoeffFit_g.append(coeff_g[1]*1000)
        ...
        ax.axhline(y=self.coeffFitCensoredDuration_e[1]*1000, ls='--', color=colorDecay)
        ax.plot(listNbins, listCoeffFit_e, color=colorDecay, label=r'$|e\rangle$ state', alpha=0.7)
        ax.set_ylabel(r'Decay rate $[\mu s^{-1}]$', color=colorDecay)
        ax.tick_params(axis='y', labelcolor=colorDecay)

        ax2 = ax.twinx()

        ax2.axhline(y=self.coeffFitCensoredDuration_g[1]*1000, ls='--', color=colorExcitation)
        ax2.plot(listNbins, listCoeffFit_g, color=colorExcitation, label=r'$|g\rangle$ state', alpha=0.7)
        ax2.set_ylabel('Excitation rate $[\mu s^{-1}]$', color=colorExcitation)
        ax2.tick_params(axis='y', labelcolor=colorExcitation)

        ax.set_xlabel('Histogram number of bins')
        ax.set_title('Fit coefficient of duration histogram')
        ax.legend()
        ...
        sys.stdout.write('\rPlot of {:{blank}} ...   done - {:.3f}s\n'.format('varation fit with Nbins', end-start, blank=nblank))
...
class MeanProbaState(StateVoltage):
    ...
    def plotMeanProba(self, save=True, savepath='', savename='Mean_Probabilities.png'):
        ...
        ax.set_xlabel('Time [ns]')
        ax.set_ylabel('Probability')
        if self.nbPhoton is None:
            titleExtension = 'rudat={}dB'.format(self.rudat)
        else:
            titleExtension = 'number of photon={}'.format(self.nbPhoton)

        title = r'Probability to be in $|e\rangle$ and $|g\rangle$ states as function of time, '
        ax.set_title(title + titleExtension)
...
class Unknown(MeanProbaState):
    ...
    def plotUnknown(self, save=True, savepath='', savename="Unknow plot"):
        nblank = MAX_BLANK_PLOT
        start = time.time()
        ...
        print(np.min(list_Y), np.max(list_Y), meanVoltage, stdVoltage)

        print(np.min(self.dictUnknown_Y['no']), np.max(self.dictUnknown_Y['no']),
              np.mean(self.dictUnknown_Y['no']), np.std(self.dictUnknown_Y['no']))
        print(np.min(self.dictUnknown_Y['pi']), np.max(self.dictUnknown_Y['pi']),
              np.mean(self.dictUnknown_Y['pi']), np.std(self.dictUnknown_Y['pi']))
        print(np.min(self.dictUnknown_Y['pio2']), np.max(self.dictUnknown_Y['pio2']),
              np.mean(self.dictUnknown_Y['pio2']), np.std(self.dictUnknown_Y['pio2']))
        ...
        for ax in axes.flat:
            ax.set(xlabel='Time integration [ns]', ylabel='Voltage [mV]')
...
class QNDness(Unknown):
    ...
    ###   PLOT THE DATA  ###########
    def colorplotAllTraject(self, type_trj, figsize=[20,40], vmin=None, vmax=None, save=True, savepath='', savename='Colorplot.png'):
        '''
    ...
    def plotAllTraject(self, percent, savepath='', type_name=''):
        ...
        # trj = self.dictAllTraject['no'][30]
        # sys.stdout.write('\r{} : plot saved {}/{}'.format('no', 1, 1))
        # traj_name = 'traj_{}__{}_{}'.format('no', 12, type_name)
        # trj.plotter()
    ...
    #######################
    ###   DO ALL  #########
    def setAllTraject_StateSeq(self, eg_center='fit', threshold='double', size=True, security=False):
        if eg_center=='mean':
    ...
    def compute_all(self, **kwargs):
        definition = kwargs['definition'] #'Me' or 'Sl'
        eg_center = kwargs['eg_center'] #'mean' or 'fit'
        threshold = kwargs['threshold'] #'mean' or 'fit' or 'double'
        security = kwargs['security'] #True or False
        size = kwargs['size'] #True or False
        saveOnlyProcessed = kwargs['saveOnlyProcessed']
        ...
        self.setSingleThreshold()
        self.fitMeanTraject()
        self.setDoubleThreshold(definition=definition)
        self.setAllTraject_StateSeq(eg_center=eg_center, threshold=threshold,
                                    size=size, security=security)

        self.setDurationList()
        self.computeJumpRate()

        # self.setHistogram_ge()
        # self.setUnitlessHistogram()
    ...
    def save_all(self, percent_plot=0.1, **kwargs):
        ...
        self.plotHistogramDuration('e',save=saveResult, savepath=savepath+'Hist_e_dtj\\', savename='Hist_e_dtj_{}.png'.format(nameExtension))
        self.plotHistogramDuration('e',save=saveResult, log=True, savepath=savepath+'Hist_e_dtj\\', savename='Hist_e_dtj_{}_log.png'.format(nameExtension))
        self.plotCumulativeFrequency('e',save=saveResult, savepath=savepath+'Hist_e_dtj\\', savename='CumFreq_e_dtj_{}.png'.format(nameExtension))

        self.plotHistogramDuration('g',save=saveResult, savepath=savepath+'Hist_g_dtj\\', savename='Hist_g_dtj_{}.png'.format(nameExtension))
        self.plotHistogramDuration('g',save=saveResult, log=True, savepath=savepath+'Hist_g_dtj\\', savename='Hist_g_dtj_{}_log.png'.format(nameExtension))
        self.plotCumulativeFrequency('g',save=saveResult, savepath=savepath+'Hist_g_dtj\\', savename='CumFreq_g_dtj_{}.png'.format(nameExtension))
        ...
        # if self.n_aver == 0:
        #     self.plotUnknown(save=saveResult, savepath=savepath+'Unknown\\', savename='Unknown_{}.png'.format(nameExtension))
        # self._plotVariationFit_withNbins()
